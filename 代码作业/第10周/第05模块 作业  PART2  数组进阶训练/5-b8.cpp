...
void calc_next(int d_cur[], int d_next[])
{
	...
	for (i = 1; i < N_NUM; i++) {
		if (i == 1) {
			...
			d_next[i] = (d_cur[i] / 2 + (d_cur[i - 1] % 2) * 5);
		}
	}
}
...
void calc_num(int pure_decimal[], int out[], int power_table[][N_NUM + 1])
{
	...
	for (i = pure_decimal[0]; i > 0; i--) {
		for (j = 1; j < N_NUM; j++) {
			out[i] += power_table[j][i] * pure_decimal[j];
		}
	}
	int now1, now2;
	for (i = pure_decimal[0]; i > 0; i--) {
		...
		out[i] = out[i] % 10;
		out[i - 1] = out[i - 1] + now2;
		out[i - 2] = out[i - 2] + now1;
	}
}
...
void input_pure_decimal(int pure_decimal[])
{
	...
	if (first_dot == '.') {
		...
		if (sum == 0) {
			cout << "输入为全0，请重输" << endl;
			for (i = 1; i < N_NUM + 1; i += 1) {
				pure_decimal[i] = { 0 };
			}
			input_pure_decimal(pure_decimal);
		}
		for (j = 1; j <= i; j++) {
			if (pure_decimal[j] == 1 || pure_decimal[j] == 0) {
				continue;
			}
			else {
